...
public class AcervoAtualizarFilmesController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        for (int i = 0; i < filmesDao.filmes.size(); i++) {
            ...
            achar.getItems().add(filmes_teste.getnome_produto());
        }

        achar.setOnAction(this::filmeEscolhido);
    ...
    {
        String nome_filme = achar.getValue();
    ...
    public void atualizarFilme(javafx.event.ActionEvent event)
    {
        ...
        for(int i=0;i<filmesDao.filmes.size();i++)
        {
            if(achar.getValue().equals(filmesDao.filmes.get(i).getnome_produto()))
            {
